...
function Section({ title }: { title: string }) {
  return (
    <div className="pt-1 border-t border-[#EAEAEA]">
      <h3 className="text-xs font-bold uppercase tracking-widest text-[#747783] pb-1">{title}</h3>
    </div>
  )
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl my-8 flex flex-col max-h-[calc(100dvh-4rem)]">
        ...
        {/* Header — shrink-0: nunca se superpone al body */}
        ...
        <form onSubmit={submit} className="flex flex-col flex-1 min-h-0">
          ...
          <div className="overflow-y-auto flex-1 p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <Field label="Valor inicial">
                <input type="number" min={0} step="any" value={form.valor_inicial} onChange={e => set("valor_inicial", e.target.value)} className={inputCls} placeholder="0" />
              </Field>
              <Field label="Adiciones">
                <input type="number" min={0} step="any" value={form.adicion} onChange={e => set("adicion", e.target.value)} className={inputCls} placeholder="0" />
              </Field>
              <Field label="Valor final">
                <input type="number" min={0} step="any" value={form.valor_final} onChange={e => set("valor_final", e.target.value)} className={inputCls} placeholder="0" />
              </Field>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="px-6 py-4 border-t border-[#EAEAEA] shrink-0 flex gap-3">
