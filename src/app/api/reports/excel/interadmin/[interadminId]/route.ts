...
export async function GET(
  ...
  // Terminología canónica: "Bolsa de Inversión" y "Cuota de Gerencia" únicamente.
  // Todos los totales se calculan desde calcInteradminFinancials — nunca desde campos legacy del DB.
  const ws2 = makeSheet(["Campo", "Valor"], kv2rows([
    ["— BOLSA DE INVERSIÓN —",              ""],
    ["Bolsa de Inversión Inicial",          n(p.valor_inicial)],
    ["Adiciones Bolsa de Inversión",        fin.adicionesBolsaInversion],
    ["Total Bolsa de Inversión",            n(fin.bienesServiciosVigente)],
    ["",                                    ""],
    ["— CUOTA DE GERENCIA —",               ""],
    ["Cuota de Gerencia Inicial",           n(p.cuota_admin_inicial)],
    ["Adiciones Cuota de Gerencia",         fin.adicionesCuotaGerencia],
    ...
    ["Facturado Bolsa de Inversión",        n(factKpis.facturadoBienes)],
    ["Facturado Cuota de Gerencia",         n(factKpis.facturadoCuota)],
  ...
  const H4 = ["Tipo", "Número", "Fecha", "Valor Total", "Valor Bolsa de Inversión",
    "Valor Cuota de Gerencia", "Número RP", "Motivo", "Observaciones / Justificación", "Usuario Registro"]
  ...
  const modRows: Row[] = [
    ...adiciones.map(a => ({
      ...
      "Valor Bolsa de Inversión":      n(a.valor_bienes_servicios),
      "Valor Cuota de Gerencia":       n(a.valor_cuota_gerencia),
